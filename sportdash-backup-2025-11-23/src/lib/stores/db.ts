...
async function load(): Promise<DB> {
  const db = await readJSON<DB>(PATH, {
    indicaties: [],
    mutaties: [],
    groepen: { list: [] },
  });
...
export async function listIndicaties(): Promise<Indicatie[]> {
  const db = await load();
  return [...db.indicaties].sort((a, b) =>
    (b.createdAt || "").localeCompare(a.createdAt || ""),
  );
}
export async function addIndicatie(
  input: Omit<Indicatie, "id" | "createdAt">,
): Promise<Indicatie> {
  const db = await load();
  const row: Indicatie = {
    id: randomUUID(),
...
export async function patchGroep(
  id: string,
  patch: Partial<Groep> & {
    addNote?: { tekst: string; auteur?: string };
    updateNote?: { id: string; tekst?: string; auteur?: string };
    removeNoteId?: string;
  },
) {
  const db = await load();
  ...
  if (patch.addNote) {
    g.notities.unshift({
      id: randomUUID(),
      tekst: patch.addNote.tekst,
      auteur: patch.addNote.auteur,
      createdAt: new Date().toISOString(),
    });
  }
  if (patch.updateNote) {
    ...
    if (n) {
      if (typeof patch.updateNote.tekst === "string")
        n.tekst = patch.updateNote.tekst;
      if (typeof patch.updateNote.auteur === "string")
        n.auteur = patch.updateNote.auteur;
    }
  }
  if (patch.removeNoteId)
    g.notities = g.notities.filter((n) => n.id !== patch.removeNoteId);
  await save(db);
  return g;
}
