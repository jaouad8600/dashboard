...
import { Package, Plus, Search, AlertTriangle, CheckCircle, Edit2, Trash2, Folder, ChevronRight, ChevronDown } from "lucide-react";
...
const FOLDER_STRUCTURE = {
  "Middelen": {
    ...
    "02. Langverblijf (Vloed)": [
      "Fitness Vloed",
      "Sportzaal Vloed",
      "Dojo", // Assuming shared or separate, but prompt listed "Dojo" once in the list but "Fitness EB/Vloed". I'll keep it simple.
      "Sportveld Vloed"
    ]
  }
};
...
export default function MaterialsPage() {
  ...
  const handleFolderSelect = (folderName: string, parent: string) => {
    // Direct mapping since folder names now match locations (mostly)
    // If parent is "01. Kortverblijf (EB)" and folder is "Fitness EB", location is "Fitness EB"
    // If folder is "Dojo", it might be ambiguous if both have Dojo, but for now we just use the folder name
    // or we can prefix it if needed. The prompt asked for "Fitness EB", "Fitness Vloed" explicitly.
  ...
  const handleSave = async () => {
    try {
      ...
      setFormData({
        name: "", category: "OVERIG", quantityTotal: 1, quantityUsable: 1, location: FORM_LOCATIONS[0], conditionStatus: "GOED"
      });
      fetchMaterials();
    } catch (error) {
