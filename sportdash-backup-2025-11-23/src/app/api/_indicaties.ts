...
// LIST
export async function listIndicaties() {
  const prisma = await getPrisma();
  if (prisma?.indicatie) {
    try {
      const rows = await prisma.indicatie.findMany({ orderBy: { createdAt: "desc" } });
...
export async function getIndicatie(id: string) {
  ...
  if (prisma?.indicatie) {
    try {
      const row = await prisma.indicatie.findUnique({ where: { id } });
      if (!row) return NextResponse.json({ error: "Niet gevonden" }, { status: 404 });
      let evl: any[] = [];
      try {
        if (prisma?.evaluatie) {
          evl = await prisma.evaluatie.findMany({
            where: { indicatieId: id },
            orderBy: { createdAt: "desc" },
          });
        }
      } catch (e) {
        console.error("[indicaties][detail][prisma-eval]", e);
      }
...
export async function updateIndicatie(req: Request, id: string) {
  ...
  if (prisma?.indicatie) {
    try {
      const updated = await prisma.indicatie.update({
        where: { id },
        data: {
          naam: patch.naam || undefined,
          type: patch.type || undefined,
          status: patch.status || undefined,
          groepId: patch.groepId ?? undefined,
          start: patch.start ? new Date(patch.start) : undefined,
          eind: patch.eind ? new Date(patch.eind) : undefined,
          opmerking: patch.opmerking ?? undefined,
          updatedAt: new Date(now),
        }
      });
...
export async function archiveIndicatie(req: Request, id: string) {
  ...
  if (prisma?.indicatie) {
    try {
      const updated = await prisma.indicatie.update({
        where: { id },
        data: { archivedAt: new Date(now), archivedReason: reason, updatedAt: new Date(now) }
      });
      return NextResponse.json(updated, { status: 200 });
    } catch (e) {
...
export async function listEvaluaties(id: string) {
  ...
  if (prisma?.evaluatie) {
    try {
      const rows = await prisma.evaluatie.findMany({
        where: { indicatieId: id },
        orderBy: { createdAt: "desc" }
      });
      return NextResponse.json(rows, { status: 200 });
  ...
  const rows = (await readEvaluaties()).filter(x=>x.indicatieId===id)
    .sort((a,b)=> new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  return NextResponse.json(rows, { status: 200 });
}
...
export async function createEvaluatie(req: Request, id: string) {
  ...
  // Verzeker dat indicatie bestaat (DB of file)
  const prisma = await getPrisma();
  if (prisma?.indicatie) {
    try {
      const ind = await prisma.indicatie.findUnique({ where: { id } });
      if (!ind) return NextResponse.json({ error: "Indicatie niet gevonden." }, { status: 404 });

      if (!prisma.evaluatie) throw new Error("Prisma model 'evaluatie' ontbreekt; fallback naar file.");
      const created = await prisma.evaluatie.create({
        data: {
          id: randomUUID(),
          indicatieId: id,
          inhoud,
      ...
      console.log("[evaluaties][create][prisma] OK", { id: created.id, indicatieId: id });
      return NextResponse.json(created, { status: 201 });
    } catch (e) {
      console.error("[evaluaties][create][prisma] fout → fallback", e);
    }
  }

  // File fallback
  const list = await readIndicaties();
  const ind2 = list.find(x=>x.id===id);
  if (!ind2) return NextResponse.json({ error: "Indicatie niet gevonden." }, { status: 404 });

  const all = await readEvaluaties();
  const item: Evaluatie = {
    id: randomUUID(),
    indicatieId: id,
    inhoud,
  ...
  all.push(item);
  await writeEvaluaties(all);
  console.log("[evaluaties][create][file] OK", { id: item.id, indicatieId: id });
  return NextResponse.json(item, { status: 201 });
}
