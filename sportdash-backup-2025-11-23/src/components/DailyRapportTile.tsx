...
export default function DailyRapportTile() {
  ...
  const data = useMemo(() => {
    ...
    const muts = safeParse<Mutatie[]>(
      localStorage.getItem("sportmutaties-v1"),
      [],
    ).filter((m) => m.dateISO === iso);
    const inds = safeParse<Indicatie[]>(
      localStorage.getItem("indicaties-v1"),
      [],
  ...
  return (
    ...
    >
      ...
      >
        <div style={card()}>
          <div style={kpi()}>T</div>
          <div style={val()}>{cnt.t}</div>
          <div style={label()}>Timeouts</div>
        </div>
        <div style={card()}>
          <div style={kpi()}>I</div>
          <div style={val()}>{cnt.i}</div>
          <div style={label()}>Incidenten</div>
        </div>
        <div style={card()}>
          <div style={kpi()}>S</div>
          <div style={val()}>{cnt.s}</div>
          <div style={label()}>Sancties</div>
        </div>
        <div style={card()}>
          <div style={kpi()}>∑</div>
          <div style={val()}>{cnt.t + cnt.i + cnt.s}</div>
          <div style={label()}>Totaal</div>
        </div>
      </div>
      ...
      >
        <div style={pill("#2563eb", "#dbeafe")}>
          <b>{data?.mutsCount ?? 0}</b> mutaties
        </div>
        <div style={pill("#16a34a", "#dcfce7")}>
          <b>{data?.indsCount ?? 0}</b> indicaties
        </div>
      </div>

      {!data?.odToday && (
        <div style={{ fontSize: 12, color: "#64748b" }}>
          Nog geen overdracht van vandaag gevonden.
        </div>
      )}
    </div>
  );
...
function pill(ink: string, bg: string) {
  return {
    fontSize: 13,
    color: ink,
    background: bg,
