...
const dump = {
  ...
  overdrachten: await prisma.overdracht.findMany({
    include: { history: true },
    orderBy: { createdAt: "desc" },
  }),
  materialen: await prisma.material.findMany({
    orderBy: { createdAt: "desc" },
  }),
  mutaties: await prisma.mutatie.findMany({
    orderBy: [{ active: "desc" }, { ts: "desc" }],
  }),
