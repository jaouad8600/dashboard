...
type Row = { id?:string; date:string; tijd?:string; titel:string; locatie?:string; afdeling?:"EB"|"VLOED" };
...
export default function PlanningPage(){
  ...
  async function save(){
    if (!sel) return;
    const base = { date: sel.date, tijd: sel.tijd||"", titel: sel.titel||"", locatie: sel.locatie||"", afdeling: sel.afdeling };
    if (!sel.id){
      const r = await fetch("/api/planning", { method:"POST", headers:{ "content-type":"application/json" }, body: JSON.stringify(base) });
      if (!r.ok){ alert("Fout bij toevoegen"); return; }
    } else {
      const r = await fetch(`/api/planning/${sel.id}`, { method:"PATCH", headers:{ "content-type":"application/json" }, body: JSON.stringify(base) });
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={()=>setSel({ date, titel:"", afdeling:"EB" })}
            className="bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded">+ Item</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow overflow-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            ...
            {filtered().length===0 && (
              <tr><td className="px-3 py-4 text-center text-gray-500" colSpan={5}>Geen items</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {sel && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow max-w-xl w-full p-4 space-y-3">
            <div className="text-lg font-semibold">{sel.id?"Bewerken":"Nieuw item"}</div>
            <div className="grid md:grid-cols-2 gap-3">
              <div>
                <div className="text-xs text-gray-500 mb-1">Datum</div>
                <input type="date" value={sel.date} onChange={e=>setSel({...sel, date:e.target.value})} className="border rounded px-2 py-1 w-full"/>
              </div>
              <div>
                <div className="text-xs text-gray-500 mb-1">Tijd</div>
                <input value={sel.tijd||""} onChange={e=>setSel({...sel, tijd:e.target.value})} placeholder="14:00" className="border rounded px-2 py-1 w-full"/>
              </div>
              ...
              <div>
                ...
                <input value={sel.locatie||""} onChange={e=>setSel({...sel, locatie:e.target.value})} className="border rounded px-2 py-1 w-full"/>
              </div>
              <div>
                <div className="text-xs text-gray-500 mb-1">Afdeling</div>
                <select value={sel.afdeling||"EB"} onChange={e=>setSel({...sel, afdeling:e.target.value as any})} className="border rounded px-2 py-1 w-full">
