...
export default function SharePointPage(){
  ...
  const update = (patch: Partial<SpConf>) => setConf(prev => ({ ...prev, ...patch }));

  return (
    <div className="p-6 space-y-4">
      <SyncService />
      <h1 className="text-2xl font-bold">SharePoint</h1>
      <p className="text-gray-600">Bewaar hier alvast je instellingen. Koppeling (Microsoft Graph) kan later worden toegevoegd.</p>
      ...
      <div className="bg-white shadow rounded-2xl p-4 space-y-3 max-w-3xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium">Document Library</label>
            <input
              className="border rounded-lg px-3 py-2 w-full"
          ...
          <div>
            <label className="block text-sm font-medium">Folder (optioneel)</label>
            <input
              className="border rounded-lg px-3 py-2 w-full"
        ...
        <div className="text-sm text-gray-500">
          ...
          <ul className="list-disc ml-5 space-y-1">
            <li>Microsoft Graph OAuth (server route) om SharePoint te lezen.</li>
            <li>.docx parser (client/server) die sportmutaties herkent en omzet.</li>
            <li>Automatisch opslaan via <code>addMutatie</code> zodat alle pagina’s live updaten.</li>
          </ul>
        </div>
      </div>
    </div>
