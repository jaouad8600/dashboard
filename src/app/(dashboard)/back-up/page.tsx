  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="rounded-2xl border bg-white p-4 grid gap-3">
        <button onClick={exportAll} className="px-3 py-2 rounded-xl border hover:bg-zinc-50">Exporteer JSON</button>
        <label className="text-sm">
          Importeer JSON:
          <input type="file" accept="application/json" onChange={importAll} className="ml-2" />
        </label>
        <div className="text-xs opacity-70">Slaat o.a. kalender, mutaties en indicaties op.</div>
      </div>
    </div>
  );
}
