...
type SumInd = { open:number; inBehandeling:number; afgerond:number; totaal:number };
type Groep = { id:string; naam:string; afdeling:"EB"|"VLOED"; kleur:"GREEN"|"YELLOW"|"ORANGE"|"RED" };
type Plan = { id:string; date:string; tijd?:string; titel:string; locatie?:string; afdeling?:"EB"|"VLOED" };
  ...
  useEffect(() => {
    ...
    const fetchOnce = async () => {
      try {
        ...
        if (!stop) { setData(j); setError(undefined); setLoading(false); }
      } catch(e:any) {
        if (!stop) { setError(e?.message || "error"); setLoading(false); }
      }
    };
    fetchOnce();
...
export default function DashboardPage(){
  const date = todayStr();
  const { data:mut, loading:lm } = usePoll<SumMut>("/api/mutaties/summary", 5000, { open:0, totaal:0 });
  const { data:ind } = usePoll<SumInd>("/api/indicaties/summary", 5000, { open:0, inBehandeling:0, afgerond:0, totaal:0 });
  const { data:rode } = usePoll<Groep[]|any>("/api/groepen/rode", 5000, []);
  const { data:plan } = usePoll<Plan[]|any>(`/api/planning?date=${date}`, 5000, []);
      ...
      <div className="grid md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl shadow p-4">
          ...
          <div className="text-xs mt-1">
            ...
            <span className="mr-2 text-indigo-700">In beh.: {ind.inBehandeling}</span>
        ...
        <div className="bg-white rounded-xl shadow p-4">
          ...
          <div className="mt-2 text-3xl font-bold">{(Array.isArray(rode)? rode.length : 0)}</div>
          <div className="text-xs text-rose-700 mt-1 truncate">{rodeNamen || "—"}</div>
        </div>
            ...
            {Array.isArray(plan) && plan.length>0
              ? plan.slice(0,4).map((p:any)=>(
                  <div key={p.id} className="text-sm flex gap-2">
                    <span className="w-14 text-gray-500">{p.tijd || "--:--"}</span>
                    <span className="font-medium">{p.titel}</span>
                    <span className="text-gray-500">· {p.locatie || "-"}</span>
                  </div>
                ))
              : <div className="text-gray-400 text-sm">Geen items voor vandaag.</div>}
          </div>
        </div>
      </div>
    </div>
