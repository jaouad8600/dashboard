...
function seed() {
  ...
  const sportmutaties = [
    { id: "s1", group: "Zijl", type: "Niet sporten", active: true, author: "Medische dienst" },
    { id: "s2", group: "Vliet", type: "Beperkt", active: true, author: "Medische dienst" },
  ];
...
export default function DevPage() {
  const [snap, setSnap] = useState({ groups:0, red:0, overdrachten:0, indicaties:0, sportmutaties:0 });
  const refresh = () => {
    ...
    setSnap({
      ...
      red,
      overdrachten: JSON.parse(localStorage.getItem("overdrachten")||"[]").length,
      indicaties: JSON.parse(localStorage.getItem("indicaties")||"[]").length,
      sportmutaties: JSON.parse(localStorage.getItem("sportmutaties")||"[]").length,
    });
  };
  useEffect(()=>{ refresh(); },[]);
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex gap-2">
        <button className="px-3 py-2 rounded bg-emerald-600 text-white" onClick={()=>{ seed(); refresh(); }}>
          Seed demo-data
        </button>
        <button className="px-3 py-2 rounded bg-red-600 text-white" onClick={()=>{ clearAll(); refresh(); }}>
          Leegmaken
        </button>
      </div>
      <div className="text-sm text-zinc-600">
        <div>groups: {snap.groups} (Rood: {snap.red})</div>
        <div>overdrachten: {snap.overdrachten}</div>
        <div>indicaties: {snap.indicaties}</div>
        <div>sportmutaties: {snap.sportmutaties}</div>
      </div>
      <p className="text-xs text-zinc-500">
        Open /dev → klik <b>Seed demo-data</b> → ga terug naar het dashboard en ververs (F5).
      </p>
    </div>
  );
}
