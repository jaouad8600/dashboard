...
export async function GET(req: NextRequest){
  ...
  const j = load(); const items = (j.indicaties?.items||[]).filter((x:any)=>!groepId || x.groepId===groepId);
...
export async function POST(req: NextRequest){
  ...
  const type=(b.type||'Algemeen').toString();
  const omschrijving=(b.omschrijving||b.opmerking||'').toString();
  const status=(b.status||'open').toString();
  if(!groepId) return NextResponse.json({ok:false,error:'groepId vereist'},{status:400});
  const j = load(); j.indicaties = j.indicaties || {items:[]};
  const item = { id: `${Date.now()}-${Math.random().toString(36).slice(2,7)}`, groepId, datum, type, omschrijving, status };
  j.indicaties.items.push(item); save(j);
