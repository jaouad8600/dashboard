...
import { loadEvents, saveEvents, makeEvent, type Tide } from "@/lib/clientStore";
 ...
 */
const WEEK_TEMPLATE: Record<number, { eb: (string|null)[]; vloed: (string|null)[] }> = {
  0: { // maandag
...
export default function SeedPage(){
  ...
  function seedByTemplate(tide: Tide){
    ...
    const filtered = current.filter(e => !(withinSameWeek(new Date(e.start), weekMon) && e.tide === tide));

    // 2) Voeg toe volgens template
    const toAdd = [];
    for(let dayIdx=0; dayIdx<5; dayIdx++){ // ma..vr
      ...
      groups.forEach((group, slotIdx) => {
        ...
        toAdd.push(makeEvent({
          title: `Sportmoment • ${group}`,
          start, end, tide, group,
        }));
      });
    }

    saveEvents([...filtered, ...toAdd]);
    setInfo(`Gezaaid: ${tide.toUpperCase()} — ${toAdd.length} momenten (${weekLabel}). /admin toont het overzicht.`);
  }
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="rounded-2xl border bg-white p-4 grid gap-3">
        <div className="text-sm opacity-80">Week: <b>{weekLabel}</b> (maandag t/m vrijdag)</div>

        <div className="grid sm:grid-cols-2 gap-3">
          <div className="rounded-xl border p-3">
            <div className="font-semibold mb-2">EB (Poel altijd)</div>
            <ul className="text-sm grid gap-1">
              <li><b>Ma–Vr</b>: 16:45 Poel</li>
            </ul>
          </div>
          ...
          <div className="rounded-xl border p-3">
            ...
            <ul className="text-sm grid gap-1">
              <li><b>Ma</b>: 17:45 Kust • 18:45 Nes • 19:45 Gaag</li>
              <li><b>Di–Vr</b>: 17:45 Kade • 18:45 Kreek • 19:45 Open</li>
            </ul>
          </div>
        </div>

        <div className="flex gap-8 flex-wrap">
          <button onClick={()=>seedByTemplate("eb")} className="px-3 py-2 rounded-xl border hover:bg-zinc-50">Seed EB (ma–vr)</button>
          <button onClick={()=>seedByTemplate("vloed")} className="px-3 py-2 rounded-xl border hover:bg-zinc-50">Seed Vloed (ma–vr)</button>
        </div>

        {info && <div className="text-sm p-3 rounded-lg border bg-zinc-50">{info}</div>}

        <div className="text-xs opacity-70">
          Her-seeden is veilig: bestaande momenten van díe week en díe tide worden eerst verwijderd. Poel staat nooit in Vloed.
        </div>
      </div>
    </div>
  );
