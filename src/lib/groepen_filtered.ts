...
export async function listGroepNotities(id: string): Promise<GroepNotitie[]> {
  const r = await fetch(`${BASE}/${id}/notities`, { cache: "no-store" }).catch(
    () => null,
  );
  if (!r || !r.ok) return [];
  const d = await r.json();
...
export async function updateGroepNotitie(
  groepId: string,
  noteId: string,
  data: { tekst?: string; auteur?: string },
) {
...
// legacy hook, no-op
export function onGroepenChange(_: () => void) {
  return () => {};
}
