...
export async function listMutaties(): Promise<Mutatie[]> {
  ...
  try { return await toJson<Mutatie[]>(res); } catch { return []; }
}
export async function createMutatie(input: {
  medewerker: string; beschrijving: string; locatie?: string; status?: MutatieStatus; severity?: Severity; datum?: string;
}): Promise<Mutatie> {
  const res = await fetch("/api/mutaties", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(input) });
  return toJson<Mutatie>(res);
}
export async function updateMutatie(id: string, patch: Partial<Mutatie>): Promise<Mutatie> {
  const res = await fetch(`/api/mutaties/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(patch) });
  return toJson<Mutatie>(res);
}
