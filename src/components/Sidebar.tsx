...
const NAV: Item[] = [
  ...
  { label: 'Weekkalender', children: [{ label: 'Schedule', href: '/kalender' }] },
...
function Row({ item, depth, activePath }:{item:Item; depth:number; activePath:string}) {
  const isActive = item.href ? activePath.startsWith(item.href) : false;
  const base = "block rounded-xl px-3 py-2 transition-colors";
  const normal = depth===0 ? "hover:bg-gray-100" : "hover:bg-gray-100 ml-5";
  const active = "bg-gray-900 text-white " + (depth>0 ? "ml-5" : "");
  if (item.href) return <Link href={item.href} className={`${base} ${isActive?active:normal}`}>{item.label}</Link>;
  return <div className={"px-3 pt-4 pb-1 text-xs uppercase tracking-wide text-gray-500 " + (depth>0?"ml-5":"")}>{item.label}</div>;
}
      ...
      <nav className="px-2 pb-6">
        {NAV.map((it, i) => (
          <div key={i} className="mb-1">
            ...
            {it.children?.map((c, j) => <Row key={j} item={c} depth={1} activePath={pathname} />)}
          </div>
        ))}
      </nav>
