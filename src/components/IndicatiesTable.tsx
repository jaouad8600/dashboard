...
export default function IndicatiesTable({ dense=false }: { dense?: boolean }) {
  ...
  const filtered = useMemo(()=>{
    ...
    return rows.filter(r=>{
      ...
      const hay = (r.naam||"") + " " + (r.type||"") + " " + (r.opmerking||"") + " " + (r.inhoud||"");
  ...
  return (
    <div className="rounded-xl border bg-white p-4 shadow-sm">
      ...
      <div className="mb-3">
        <TableFilters q={q} onQ={setQ} status={status} onStatus={setStatus} placeholder="Zoek naam, type, opmerking…" />
      </div>
        ...
        <table className={`w-full text-sm ${dense?"":"table-auto"}`}>
          ...
          <tbody>
            {filtered.length===0 && !loading && (
              <tr><td colSpan={5} className="p-4 text-center text-zinc-500">Geen indicaties gevonden.</td></tr>
            )}
            {filtered.map(r=>(
        ...
        return (
          <div className="mt-3 rounded-lg border bg-zinc-50 p-3">
            <div className="text-sm">
              ...
              <div className="text-xs text-zinc-500 mb-2">{item.type||"—"} • {item.status}</div>
