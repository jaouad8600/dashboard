  ...
  if (!data.groepen?.list || data.groepen.list.length === 0) {
    const list: Groep[] = [
      ...EB_GROEPEN.map((naam) => ({
        id: uuid(),
        naam,
        afdeling: "EB" as const,
        kleur: "GREEN" as Kleur,
        notities: [],
      })),
      ...VLOED_GROEPEN.map((naam) => ({
        id: uuid(),
        naam,
        afdeling: "VLOED" as const,
        kleur: "GREEN" as Kleur,
        notities: [],
      })),
    ];
...
export async function setGroepKleur(
  id: string,
  kleur: Kleur,
): Promise<Groep | null> {
...
export async function getIndicaties(): Promise<Indicatie[]> {
  const data = await readJSON();
  return data.indicaties.sort((a, b) =>
    (b.updatedAt ?? b.createdAt).localeCompare(a.updatedAt ?? a.createdAt),
  );
}
export async function addIndicatie(
  input: Partial<Indicatie>,
): Promise<Indicatie> {
  const data = await readJSON();
...
export async function patchIndicatie(
  id: string,
  patch: Partial<Indicatie>,
): Promise<Indicatie | null> {
  const data = await readJSON();
