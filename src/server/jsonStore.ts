...
export type Indicatie = {
  ...
  datum?: string; inhoud?: string; aanmelder?: string; docUrl?: string;
};
...
export type Mutatie = {
  ...
  type?: string;
  status?: 'Open'|'In behandeling'|'Afgerond';
  tijd?: string; omschrijving?: string;
};
...
async function ensureFile() {
  try { await access(filePath, constants.F_OK); }
    ...
    const empty: AppData = { groepen:{list:[]}, indicaties:[], mutaties:[], overdrachten:[], planning:{} };
