...
function writeJSON<T>(key: string, value: T) {
  ...
  try {
    ...
    window.dispatchEvent(
      new CustomEvent("sportdash-sync", { detail: { key } }),
    );
...
export function useLiveList<T = any>(key: string, initial: T[]) {
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === key) setList(readJSON<T[]>(key, initial));
    };
    const onCustom = (e: Event) => {
      const de = e as CustomEvent;
      if (de?.detail?.key === key || de?.detail?.key === "*")
        setList(readJSON<T[]>(key, initial));
    };
    ...
    if ("BroadcastChannel" in window) {
      ...
      bc.onmessage = (msg) => {
        if (msg?.data?.key === key || msg?.data?.key === "*")
          setList(readJSON<T[]>(key, initial));
      };
    }
    return () => {
      window.removeEventListener("storage", onStorage);
      window.removeEventListener("sportdash-sync", onCustom as EventListener);
      bc?.close();
    };
  }, [key]);
  const save = (next: T[] | ((prev: T[]) => T[])) => {
    const resolved =
      typeof next === "function"
        ? (next as any)(readJSON<T[]>(key, initial))
        : next;
...
export function useLiveObject<T extends AnyRec = AnyRec>(
  key: string,
  initial: T,
) {
  ...
  useEffect(() => {
    if (!isClient) return;
    const onStorage = (e: StorageEvent) => {
      if (e.key === key) setObj(readJSON<T>(key, initial));
    };
    const onCustom = (e: Event) => {
      const de = e as CustomEvent;
      if (de?.detail?.key === key || de?.detail?.key === "*")
        setObj(readJSON<T>(key, initial));
    };
    window.addEventListener("storage", onStorage);
    window.addEventListener("sportdash-sync", onCustom as EventListener);
    let bc: BroadcastChannel | null = null;
...
export const useGroupColors = () =>
  useLiveObject<Record<string, string>>(KEY_COL, {});
...
export function addMutatie(item: any) {
  const list = readJSON<any[]>(KEY_MUT, []);
  list.unshift(item);
  writeJSON(KEY_MUT, list);
}
export function addIndicatie(item: any) {
  const list = readJSON<any[]>(KEY_IND, []);
  list.unshift(item);
  writeJSON(KEY_IND, list);
}
export function addOverdracht(item: any) {
  const list = readJSON<any[]>(KEY_OVD, []);
  list.unshift(item);
  writeJSON(KEY_OVD, list);
}
export function addMelding(item: any) {
  const list = readJSON<any[]>(KEY_MSG, []);
  list.unshift(item);
  writeJSON(KEY_MSG, list);
}
export function setEventsAll(list: any[]) {
  writeJSON(KEY_EVT, list);
}
