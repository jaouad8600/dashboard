...
function emitChange() {
  if (typeof window !== "undefined")
    window.dispatchEvent(new CustomEvent("overdrachten:changed"));
}
...
export async function createOverdracht(input: {
  title?: string;
  body: string;
  author?: string;
}) {
...
export async function patchOverdracht(
  id: string,
  patch: { title?: string; body?: string; author?: string },
) {
...
export function onOverdrachtenChange(cb: () => void) {
  ...
  return () => {
    if (typeof window !== "undefined") {
      window.removeEventListener("overdrachten:changed", h as EventListener);
    }
  };
}
